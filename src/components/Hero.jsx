...
const linkConfig = [
  ...
  {
    key: 'htb',
    label: { es: 'HackTheBox', en: 'HackTheBox' },
    icon: (
      <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 24 24">
...
function Hero() {
  ...
  const cvLabel = lang === 'es' ? 'Descargar CV' : 'Download Resume'
  const cvLabelSm = lang === 'es' ? 'CV' : 'CV'
        ...
        <div className="flex flex-col gap-4 text-center md:text-left w-full">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-2">
            {linkConfig.map(({ key, label, icon }) =>
              p.links[key] ? (
                <a
                  ...
                  title={label[lang]}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-surface2 border border-surface2 text-secondary hover:text-primary hover:border-primary/50 transition-all text-sm font-medium"
                >
                  {icon}
                  <span className="hidden min-[400px]:inline">{label[lang]}</span>
                </a>
              ) : null
            )}
            <a
            ...
            >
              ...
              <span className="hidden min-[400px]:inline">{cvLabel}</span>
              <span className="min-[400px]:hidden">{cvLabelSm}</span>
            </a>
          </div>
        </div>
      </div>
