...
const URL = 'https://estebanzarate.vercel.app'
const TEXT = 'Esteban Zárate — Cybersecurity Analyst'
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col-reverse items-end gap-2">
      {open && ACTIONS.map((action, i) => (
        ...
        >
          <span className="text-xs text-secondary bg-surface border border-surface2 px-2 py-0.5 rounded shadow-sm whitespace-nowrap">
            {action.id === 'instagram' ? 'Copiar URL (pegar en Instagram)' : action.label}
            {action.id === 'copy' && copied ? ' ✓' : ''}
            {action.id === 'instagram' && copied ? ' ✓' : ''}
          </span>
          <button
      ...
      <button
        ...
        aria-label={open ? 'Cerrar compartir' : 'Compartir'}
        aria-expanded={open}
