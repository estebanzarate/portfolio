...
function HTBChallenges() {
  const { lang } = useLanguage();
  const t = translations[lang].challenges;
  const { statistics, challenges } = challengesData;
  ...
  const filtered = useMemo(
    () =>
      challenges.filter((c) => {
        ...
        const matchOwned =
          filterOwned === "All" ||
          (filterOwned === "owned" ? c.is_owned : !c.is_owned);
        const matchSearch = c.name.toLowerCase().includes(search.toLowerCase());
        return matchDiff && matchCat && matchOwned && matchSearch;
      }),
    [challenges, filterDiff, filterCat, filterOwned, search],
  );
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      let av = a[sortKey],
        bv = b[sortKey];
      if (sortKey === "writeup") {
        av = writeupsData.challenges?.[String(a.id)]?.writeup ?? "";
        bv = writeupsData.challenges?.[String(b.id)]?.writeup ?? "";
      }
  ...
  return (
    <section id="challenges" className="flex flex-col gap-6 scroll-mt-20">
      <div className="border-l-4 border-primary pl-4">
        <h2 className="text-2xl font-bold text-light">
          HackTheBox <span className="text-primary">Challenges</span>
        </h2>
        {statistics.total > 0 && (
          <p className="text-secondary text-sm mt-1">
            {t.subtitle}:{" "}
            {new Date(challengesData.last_updated).toLocaleDateString(
              lang === "es" ? "es-AR" : "en-US",
            )}
          </p>
        )}
          ...
          {!hasResults ? (
            ...
            <>
              ...
              <div className="flex flex-col gap-2 md:hidden">
                {paginated.map((c) => {
                  const writeup =
                    writeupsData.challenges?.[String(c.id)]?.writeup ?? null;
                  return (
                    <div
                      key={c.id}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        <span
                          className={`px-2 py-0.5 rounded border text-xs font-medium ${difficultyColor[c.difficulty] ?? "text-secondary border-surface2"}`}
                        >
                          {c.difficulty}
                        </span>
                        <span
                          className={`text-sm font-bold ${c.is_owned ? "text-success" : "text-secondary"}`}
                        >
                          {c.is_owned ? "✓" : "✗"}
                        </span>
                      </div>
              ...
              <div className="hidden md:block overflow-x-auto rounded-lg border border-surface2">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {paginated.map((c, i) => {
                      const writeup =
                        writeupsData.challenges?.[String(c.id)]?.writeup ??
                        null;
                      return (
                        <tr
                          key={c.id}
